...
def test_fade_long_fires_on_sweep_not_break():
    ...
    sig = fade_long(df)
    assert bool(sig.iloc[0]) is True, "sweep debe disparar fade long"
    assert bool(sig.iloc[1]) is False, "close-below (break) NO debe disparar fade long"
