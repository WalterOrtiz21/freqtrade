...
# =============================================================================
import os
trade_logger = logging.getLogger('LorentzianSuperTrendRoll.trades')
...
class LorentzianSuperTrendRoll(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def _normalize_rolling(self, series: Union[pd.Series, np.ndarray], window: int) -> pd.Series:
        """Bounded-window variant of Pine normalize() — past-only, per-pair.

        Uses a rolling last-`window` min/max instead of expanding() so the
        normalization span is identical in backtest and in live. In live/dry
        freqtrade only fetches a bounded candle window from the exchange
        (Bitget caps at ~1000 candles/pair), while backtest sees the full local
        history; expanding() would normalize over years in backtest and over
        ~weeks in live, so the two diverge. A fixed rolling window (<= the live
        cap) makes both converge. min_periods=1 keeps warmup behaviour identical
        to expanding() until `window` bars of history have accumulated.
        """
        if isinstance(series, np.ndarray):
            series = pd.Series(series)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        wt2 = ta.SMA(wt1, timeperiod=4)
        wt_diff = wt1 - wt2
        # Bounded rolling normalize(window=500): per-pair, past-only, live==backtest parity
        dataframe['f2_norm'] = self._normalize_rolling(wt_diff, window=500)
        ...
        # Bounded rolling normalize(window=500): per-pair, past-only, live==backtest parity
        dataframe['f3_norm'] = self._normalize_rolling(cci_smooth, window=500)
